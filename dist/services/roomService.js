...
class RoomService {
     ...
     */
    static async isRoomOwner(roomId, username) {
        try {
    ...
    static async getRoomInfo(roomId) {
        try {
            const results = await (0, db_1.query)(`SELECT r.room_id, r.room_name, r.owner_name, r.password, r.is_active, r.is_delete, r.created_at, 
     ...
     */
    static async editRoom(request, changedBy) {
        try {
            ...
            const currentRoom = await (0, db_1.query)('SELECT * FROM rooms WHERE room_id = ?', [request.roomId]);
     ...
     */
    static async removePassword(roomId, username, reason) {
     ...
     */
    static async toggleActiveStatus(roomId, username, isActive, reason) {
        return await this.editRoom({
            roomId,
            isActive,
            reason
     ...
     */
    static async toggleDeleteStatus(roomId, username, isDelete, reason) {
        return await this.editRoom({
            roomId,
            isDelete,
            reason
 ...
 * Activate a room
 */
    static async activateRoom(roomId, username, reason) {
        try {
     ...
     * Deactivate a room
     */
    static async deactivateRoom(roomId, username, reason) {
        try {
